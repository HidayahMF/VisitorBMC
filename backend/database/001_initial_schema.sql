...
CREATE TABLE Visitors (
    ...
    CONSTRAINT FK_Visitors_Companies   FOREIGN KEY (CompanyId) REFERENCES Companies(Id)
);
GO

CREATE INDEX IX_Visitors_VisitorName  ON Visitors (VisitorName);
CREATE INDEX IX_Visitors_CompanyId    ON Visitors (CompanyId);
GO
...
CREATE TABLE Visits (
    ...
    CONSTRAINT FK_Visits_Companies     FOREIGN KEY (CompanyId) REFERENCES Companies(Id),
    CONSTRAINT FK_Visits_CreatedBy     FOREIGN KEY (CreatedBy) REFERENCES Users(Id),
    CONSTRAINT FK_Visits_CheckedOutBy  FOREIGN KEY (CheckedOutBy) REFERENCES Users(Id),
...
CREATE INDEX IX_Visits_CompanyId      ON Visits (CompanyId);
GO

-- ============================================================
-- 5. VisitVisitors (junction table)
-- ============================================================
-- Junction table: links Visits to Visitors (Many-to-Many).
-- SafetyStatus is intentionally NOT stored here.
-- It is computed at runtime based on the latest VisitorInductionRecords
...
CREATE TABLE VisitVisitors (
    ...
    CONSTRAINT FK_VisitVisitors_Visits     FOREIGN KEY (VisitId) REFERENCES Visits(Id),
    CONSTRAINT FK_VisitVisitors_Visitors   FOREIGN KEY (VisitorId) REFERENCES Visitors(Id)
);
GO

CREATE INDEX IX_VisitVisitors_VisitId     ON VisitVisitors (VisitId);
CREATE INDEX IX_VisitVisitors_VisitorId   ON VisitVisitors (VisitorId);
GO
...
-- ============================================================
-- 7. SafetyInductionContents
-- ============================================================
CREATE TABLE SafetyInductionContents (
    Id                      INT IDENTITY(1,1)   NOT NULL,
    ...
    CONSTRAINT FK_SICContents_SafetyInductions    FOREIGN KEY (SafetyInductionId) REFERENCES SafetyInductions(Id),
    CONSTRAINT CK_SICContents_ContentType         CHECK (ContentType IN ('IMAGE', 'VIDEO'))
);
GO

CREATE INDEX IX_SICContents_SafetyInductionId ON SafetyInductionContents (SafetyInductionId);
GO

-- ============================================================
-- 8. VisitorInductionRecords
-- ============================================================
...
CREATE TABLE VisitorInductionRecords (
    Id                      INT IDENTITY(1,1)   NOT NULL,
    VisitorId               INT                 NOT NULL,
    VisitId                 INT                 NOT NULL,
    SafetyInductionId       INT                 NOT NULL,
    ...
    CONSTRAINT FK_VIRecords_Visitors                FOREIGN KEY (VisitorId) REFERENCES Visitors(Id),
    CONSTRAINT FK_VIRecords_Visits                  FOREIGN KEY (VisitId) REFERENCES Visits(Id),
    CONSTRAINT FK_VIRecords_SafetyInductions        FOREIGN KEY (SafetyInductionId) REFERENCES SafetyInductions(Id),
    CONSTRAINT FK_VIRecords_CreatedBy               FOREIGN KEY (CreatedBy) REFERENCES Users(Id),
    CONSTRAINT CK_VIRecords_InductionVersion        CHECK (InductionVersion > 0)
);
GO

-- Index for fast validity checks and history queries
CREATE INDEX IX_VIRecords_VisitorId                ON VisitorInductionRecords (VisitorId);
CREATE INDEX IX_VIRecords_VisitorId_SafetyInductionId_CompletedAt
    ON VisitorInductionRecords (VisitorId, SafetyInductionId, CompletedAt);
CREATE INDEX IX_VIRecords_ValidUntil               ON VisitorInductionRecords (ValidUntil);
-- FK index for audit queries: find inductions supervised by a specific Security user
CREATE INDEX IX_VIRecords_CreatedBy               ON VisitorInductionRecords (CreatedBy);
GO
