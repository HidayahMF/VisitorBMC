    ...
    <div className="admin-page user-management-page">
      ...
      <form onSubmit={create} className="access-form">
        ...
        <div className="access-form-grid">
          ...
          <div className="form-field role-field"><label htmlFor="user-role">{t('userManagement.role')}</label><select id="user-role" value={role} onChange={event => setRole(event.target.value as ManagedUser['role'])} disabled={saving}><option value="SECURITY">{t('userManagement.security')}</option><option value="ADMIN">{t('userManagement.admin')}</option></select></div>
      ...
      <section className="user-list-section" aria-labelledby="user-list-title">
        ...
        {loading ? <p className="page-loading" role="status">{t('common.loading')}</p> : users.length === 0 ? <EmptyState message={t('userManagement.empty')} /> : <div className="user-list">{users.map(user => <article className="user-row" key={user.id}><div className="user-identity"><strong>{user.name || user.username}</strong><span>{t('userManagement.nip')}: {user.username}</span></div><div className="user-current-role"><span className="role-badge">{user.role === 'ADMIN' ? t('userManagement.admin') : t('userManagement.security')}</span></div><div><span className={`status-badge ${user.isActive ? 'bg-green-100' : 'bg-gray-100'}`}>{user.isActive ? t('common.active') : t('common.inactive')}</span></div><div className="user-role-control"><label className="sr-only" htmlFor={`role-${user.id}`}>{t('userManagement.changeRole')} {user.name}</label><select id={`role-${user.id}`} value={user.role} disabled={saving} onChange={event => void change(user, { role: event.target.value as ManagedUser['role'] })}><option value="SECURITY">{t('userManagement.security')}</option><option value="ADMIN">{t('userManagement.admin')}</option></select></div><button type="button" className={user.isActive ? 'warning-button' : 'secondary-button'} disabled={saving} onClick={() => void change(user, { isActive: !user.isActive })}>{user.isActive ? t('common.deactivate') : t('common.activate')}</button></article>)}</div>}
      </section>
    </div>
  </Layout>;
}
