...
describe('CompaniesPage', () => {
  ...
  it('renders localized header, result count, status, and edit affordance', async () => { renderPage(); expect(await screen.findByRole('heading', { name: 'Perusahaan yang sudah terdaftar' })).toBeInTheDocument(); expect(screen.getByText('Kelola perusahaan yang terdaftar untuk kunjungan pengunjung.')).toBeInTheDocument(); expect(screen.getByText('1 perusahaan')).toBeInTheDocument(); expect(screen.getAllByText('Aktif').length).toBeGreaterThan(0); expect(screen.getAllByRole('button', { name: 'Bakrie Autoparts' }).length).toBeGreaterThan(0); });
