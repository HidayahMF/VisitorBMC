...
export function CompaniesPage() {
  ...
  async function handleDelete(company: Company) { if (!await confirmAction(`${t('common.delete')} "${company.companyName}"?\nSemua pengunjung, kunjungan, dan riwayat induction perusahaan ini akan ikut terhapus permanen.`)) return; setActionLoading(company.id); setError(''); try { await deleteCompany(company.id); await load(); } catch (cause) { setError(userFacingError(cause, language)); } finally { setActionLoading(null); } }
  ...
  return <Layout><div className="companies-page">
    <header className="page-header-compact"><div><p className="page-eyebrow">{t('navigation.administration')}</p><h1>{language === 'id' ? 'Perusahaan yang sudah terdaftar' : t('companies.title')}</h1><p>{t('companies.description')}</p></div><button type="button" onClick={() => navigate('/companies/new')} className="primary-button">+ {t('companies.add')}</button></header>
    ...
    {error ? <ErrorState message={error} onRetry={load} /> : loading ? <p className="page-loading">{t('common.loading')}</p> : companies.length === 0 ? <div className="company-empty"><EmptyState message={emptyMessage} />{!search.trim() && <><p>{t('companies.emptyHint')}</p><button type="button" onClick={() => navigate('/companies/new')} className="primary-button">+ {t('companies.add')}</button></>}</div> : <>
       <div className="companies-table-wrap"><table className="companies-table"><colgroup><col /><col className="companies-status-col" />{(user?.role === 'ADMIN' || user?.role === 'SECURITY') && <col className="companies-action-col" />}</colgroup><thead><tr><th>{t('companies.company')}</th><th>{t('companies.status')}</th>{(user?.role === 'ADMIN' || user?.role === 'SECURITY') && <th className="text-right">{t('companies.action')}</th>}</tr></thead><tbody>{companies.map(company => <tr key={company.id}><td>{user?.role === 'ADMIN' ? <button type="button" onClick={() => navigate(`/companies/${company.id}/edit`)} className="company-name-link">{company.companyName}</button> : <span className="company-name">{company.companyName}</span>}</td><td><CompanyStatus active={company.isActive} label={company.isActive ? t('companies.active') : t('companies.inactive')} /></td>{(user?.role === 'ADMIN' || user?.role === 'SECURITY') && <td className="text-right"><button type="button" disabled={actionLoading !== null} onClick={() => void handleToggleStatus(company)} className="row-action">{actionLoading === company.id ? t('common.update') : company.isActive ? t('common.deactivate') : t('common.activate')}</button><button type="button" className="danger-button" disabled={actionLoading !== null} onClick={() => void handleDelete(company)}>{t('common.delete')}</button></td>}</tr>)}</tbody></table></div>
       <div className="mobile-record-list companies-mobile-list">{companies.map(company => <article key={company.id} className="mobile-record-card"><div className="mobile-record-heading"><div>{user?.role === 'ADMIN' ? <button type="button" onClick={() => navigate(`/companies/${company.id}/edit`)} className="company-name-link">{company.companyName}</button> : <strong>{company.companyName}</strong>}</div><CompanyStatus active={company.isActive} label={company.isActive ? t('companies.active') : t('companies.inactive')} /></div>{(user?.role === 'ADMIN' || user?.role === 'SECURITY') && <div className="mobile-record-actions"><button type="button" disabled={actionLoading !== null} onClick={() => void handleToggleStatus(company)} className="row-action">{actionLoading === company.id ? t('common.update') : company.isActive ? t('common.deactivate') : t('common.activate')}</button><button type="button" className="danger-button" disabled={actionLoading !== null} onClick={() => void handleDelete(company)}>{t('common.delete')}</button></div>}</article>)}</div>
